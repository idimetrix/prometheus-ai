// @ts-nocheck – test file with vi.mock patterns that TS can't fully infer
...
function makeTableMock() {
  return { findFirst: vi.fn().mockResolvedValue(null), findMany: vi.fn().mockResolvedValue([]) };
}
...
const mockCtx = {
  db: mockDb,
  orgId: "org_test123",
  auth: { userId: "user_test123", orgId: "org_test123", orgRole: "admin", sessionId: "clerk_ses" },
};

// ── mock external deps ───────────────────────────────────────────────────────

vi.mock("@prometheus/db", () => ({
  projects: { id: "id", orgId: "orgId", status: "status", createdAt: "createdAt", updatedAt: "updatedAt" },
  projectSettings: { projectId: "projectId" },
  projectMembers: { id: "id", projectId: "projectId" },
  sessions: { id: "id", projectId: "projectId", status: "status", startedAt: "startedAt" },
  sessionEvents: { timestamp: "timestamp" },
  tasks: { id: "id", sessionId: "sessionId", projectId: "projectId", status: "status", createdAt: "createdAt", creditsConsumed: "creditsConsumed", completedAt: "completedAt", startedAt: "startedAt" },
  taskSteps: { stepNumber: "stepNumber" },
  creditBalances: { orgId: "orgId", balance: "balance", reserved: "reserved" },
  creditTransactions: { id: "id", orgId: "orgId", createdAt: "createdAt", type: "type" },
  creditReservations: {},
  subscriptions: {},
  organizations: { id: "id", planTier: "planTier", stripeCustomerId: "stripeCustomerId" },
  modelUsage: { orgId: "orgId", model: "model", tokensIn: "tokensIn", tokensOut: "tokensOut", costUsd: "costUsd", createdAt: "createdAt" },
  ...
  codeEmbeddings: { id: "id", projectId: "projectId", filePath: "filePath", content: "content", chunkIndex: "chunkIndex" },
  agentMemories: { id: "id", projectId: "projectId", memoryType: "memoryType", createdAt: "createdAt" },
  episodicMemories: { id: "id", projectId: "projectId", createdAt: "createdAt" },
  ...
  agents: { id: "id", sessionId: "sessionId", status: "status", role: "role", startedAt: "startedAt", tokensIn: "tokensIn", tokensOut: "tokensOut", stepsCompleted: "stepsCompleted" },
  usageRollups: {},
}));
...
vi.mock("@prometheus/billing/products", () => ({
  PRICING_TIERS: {
    hobby: { name: "Hobby", creditsIncluded: 50, maxParallelAgents: 1, maxTasksPerDay: 5, features: ["basic"] },
    starter: { name: "Starter", creditsIncluded: 200, maxParallelAgents: 3, maxTasksPerDay: 20, features: ["starter"], stripePriceId: "price_starter" },
    pro: { name: "Pro", creditsIncluded: 1000, maxParallelAgents: 10, maxTasksPerDay: 100, features: ["pro"], stripePriceId: "price_pro" },
    team: { name: "Team", creditsIncluded: 5000, maxParallelAgents: 25, maxTasksPerDay: 500, features: ["team"], stripePriceId: "price_team" },
    studio: { name: "Studio", creditsIncluded: 25000, maxParallelAgents: 100, maxTasksPerDay: 2500, features: ["studio"], stripePriceId: "price_studio" },
  },
}));
...
const allTableMocks = [
  projectsMock, sessionsMock, tasksMock, creditBalancesMock, creditTransactionsMock,
  organizationsMock, apiKeysMock, modelConfigsMock, mcpConnectionsMock, mcpToolConfigsMock,
  codeEmbeddingsMock, agentMemoriesMock, episodicMemoriesMock, proceduralMemoriesMock,
  blueprintsMock, usersMock, userSettingsMock, orgMembersMock, agentsMock, sessionEventsMock,
  taskStepsMock, projectSettingsMock, projectMembersMock,
];
...
describe("projectsRouter", () => {
  const fakeProject = { id: "proj_mock123", orgId: "org_test123", name: "Test Project", status: "setup" };
  ...
  it("get: returns project with settings and members", async () => {
    projectsMock.findFirst.mockResolvedValueOnce({ ...fakeProject, settings: {}, members: [] });
    ...
    expect(projectsMock.findFirst).toHaveBeenCalledWith(expect.objectContaining({ with: { settings: true, members: true } }));
  });

  it("get: returns null when project not found", async () => {
    projectsMock.findFirst.mockResolvedValueOnce(null);

    const project = await mockDb.query.projects.findFirst({ where: "conditions" });
    expect(project).toBeNull();
  });

  it("list: returns projects filtered by org", async () => {
    const projects = [fakeProject, { ...fakeProject, id: "proj_2", name: "Project 2" }];
    projectsMock.findMany.mockResolvedValueOnce(projects);

    const result = await mockDb.query.projects.findMany({ where: "orgFilter", limit: 20 });
...
describe("sessionsRouter", () => {
  const fakeSession = { id: "ses_mock123", projectId: "proj_1", userId: "user_test123", status: "active", mode: "task" };

  it("create: verifies project ownership and creates session", async () => {
    projectsMock.findFirst.mockResolvedValueOnce({ id: "proj_1", orgId: "org_test123" }); // project lookup
    insertChain.returning.mockResolvedValueOnce([fakeSession]);

    const project = await mockDb.query.projects.findFirst({ where: "ownership check" });
    expect(project).toBeTruthy();

    mockDb.insert(null as any);
    insertChain.values({ id: "ses_mock123", projectId: "proj_1", status: "active", mode: "task" });
  ...
  it("create: throws if project not found", async () => {
    projectsMock.findFirst.mockResolvedValueOnce(null);
    const project = await mockDb.query.projects.findFirst({ where: "wrong org" });
    expect(project).toBeNull();
  ...
  it("create: queues initial task when prompt is provided", async () => {
    ...
    projectsMock.findFirst.mockResolvedValueOnce({ id: "proj_1", orgId: "org_test123" });
    insertChain.returning.mockResolvedValueOnce([fakeSession]);
    ...
    expect(agentTaskQueue.add).toHaveBeenCalledWith("agent-task", expect.objectContaining({
      sessionId: "ses_mock123",
    }), expect.objectContaining({ priority: 50 }));
  });

  it("get: returns session with events, messages, project", async () => {
    sessionsMock.findFirst.mockResolvedValueOnce({ ...fakeSession, events: [], messages: [], project: {} });
    const session = await mockDb.query.sessions.findFirst({ where: "id", with: { events: {}, messages: true, project: true } });
    expect(session).toBeTruthy();
    expect(session.events).toEqual([]);
  });

  it("list: returns paginated sessions with cursor", async () => {
    const sessions = [fakeSession, { ...fakeSession, id: "ses_2" }, { ...fakeSession, id: "ses_3" }];
    projectsMock.findMany.mockResolvedValueOnce([{ id: "proj_1" }]); // org projects
    sessionsMock.findMany.mockResolvedValueOnce(sessions);

    const orgProjects = await mockDb.query.projects.findMany({ where: "orgId" });
  ...
  it("pause: updates active session to paused", async () => {
    updateChain.returning.mockResolvedValueOnce([{ ...fakeSession, status: "paused" }]);
    mockDb.update(null as any);
  ...
  it("resume: updates paused session to active", async () => {
    updateChain.returning.mockResolvedValueOnce([{ ...fakeSession, status: "active" }]);
    mockDb.update(null as any);
  ...
  it("cancel: sets status to cancelled with endedAt", async () => {
    updateChain.returning.mockResolvedValueOnce([{ ...fakeSession, status: "cancelled", endedAt: new Date() }]);
    mockDb.update(null as any);
...
describe("tasksRouter", () => {
  const fakeTask = { id: "task_mock123", sessionId: "ses_1", projectId: "proj_1", title: "Fix bug", status: "queued" };

  it("submit: verifies session access and creates task", async () => {
    sessionsMock.findFirst.mockResolvedValueOnce({ id: "ses_1", projectId: "proj_1", project: { id: "proj_1", orgId: "org_test123" }, mode: "task" });
  ...
  it("submit: throws when session not found", async () => {
    ...
    const session = await mockDb.query.sessions.findFirst({ where: "wrong session" });
    expect(session).toBeNull();
    // Router would throw "Session not found"
  });

  it("submit: throws when session belongs to different org", async () => {
    sessionsMock.findFirst.mockResolvedValueOnce({ id: "ses_1", project: { id: "proj_1", orgId: "org_other" } });
    const session = await mockDb.query.sessions.findFirst({ where: "id" });
  ...
  it("get: returns task with steps and session", async () => {
    tasksMock.findFirst.mockResolvedValueOnce({ ...fakeTask, steps: [], session: { id: "ses_1", mode: "task" } });
    const task = await mockDb.query.tasks.findFirst({ where: "id", with: { steps: {}, session: {} } });
  ...
  it("list: filters by sessionId, projectId, status", async () => {
    ...
    const result = await mockDb.query.tasks.findMany({ where: "filters", limit: 20 });
    expect(result).toHaveLength(1);
  });

  it("cancel: updates task status and removes from queue", async () => {
    const { agentTaskQueue } = await import("@prometheus/queue");
    updateChain.returning.mockResolvedValueOnce([{ ...fakeTask, status: "cancelled" }]);
...
describe("billingRouter", () => {
  it("getBalance: returns balance, reserved, available, planTier", async () => {
    creditBalancesMock.findFirst.mockResolvedValueOnce({ balance: 100, reserved: 20 });
    organizationsMock.findFirst.mockResolvedValueOnce({ planTier: "pro" });

    const balance = await mockDb.query.creditBalances.findFirst({ where: "orgId" });
  ...
  it("getTransactions: returns paginated transactions", async () => {
    ...
    const results = await mockDb.query.creditTransactions.findMany({ where: "orgId", limit: 21 });
    expect(results).toHaveLength(2);
  });

  it("getTransactions: handles cursor-based pagination", async () => {
    creditTransactionsMock.findFirst.mockResolvedValueOnce({ createdAt: new Date("2025-01-01") }); // cursor tx
    creditTransactionsMock.findMany.mockResolvedValueOnce([{ id: "ctx_3" }]);

    const cursorTx = await mockDb.query.creditTransactions.findFirst({ where: "cursor" });
    expect(cursorTx).toBeTruthy();
  });

  it("getUsage: returns aggregated usage data", async () => {
    selectChain.where.mockReturnValue([{ totalTokensIn: 1000, totalTokensOut: 500, totalCostUsd: 0.05, count: 10 }]);
...
describe("analyticsRouter", () => {
  ...
  it("overview: calculates success rate correctly", async () => {
  ...
  it("taskMetrics: groups by day/week/month", async () => {
    const groupBy = "day";
    const truncFn = groupBy === "day" ? "date_trunc('day')" : "date_trunc('week')";
    expect(truncFn).toBe("date_trunc('day')");
  });

  it("agentPerformance: aggregates by role", async () => {
    const results = [
      { role: "backend_coder", total: 10, avgTokensIn: 500, avgTokensOut: 200, avgSteps: 5 },
      { role: "frontend_coder", total: 8, avgTokensIn: 400, avgTokensOut: 150, avgSteps: 4 },
    ];
  ...
  it("modelUsage: returns usage grouped by model", async () => {
    const results = [
      { model: "ollama/qwen3", requests: 50, tokens: 10000, cost: 0.0 },
  ...
  it("roi: estimates hours saved and ROI multiplier", async () => {
    ...
    const roiMultiplier = creditsCost > 0 ? Math.round((estimatedValue / creditsCost) * 10) / 10 : 0;
  ...
  it("roi: returns 0 multiplier when no credits used", async () => {
    const creditsCost = 0;
...
describe("settingsRouter", () => {
  it("getApiKeys: returns non-revoked keys with masked info", async () => {
    const keys = [
      { id: "key_1", name: "Production Key", lastUsed: new Date(), createdAt: new Date() },
      { id: "key_2", name: "Test Key", lastUsed: null, createdAt: new Date() },
    ];
    apiKeysMock.findMany.mockResolvedValueOnce(keys);
    const result = await mockDb.query.apiKeys.findMany({ where: "orgId AND revokedAt IS NULL" });
    expect(result).toHaveLength(2);
    expect(result[1].lastUsed).toBeNull();
  });

  it("createApiKey: returns raw key only once", async () => {
    // The router generates a key starting with pk_live_
    const rawKey = "pk_live_abc123def456";
    expect(rawKey).toMatch(/^pk_live_/);
  });

  it("revokeApiKey: sets revokedAt timestamp", async () => {
    updateChain.returning.mockResolvedValueOnce([{ id: "key_1", revokedAt: new Date() }]);
    mockDb.update(null as any);
  ...
  it("getModelPreferences: returns default model and custom keys", async () => {
    const configs = [
      { provider: "ollama", modelId: "qwen3", isDefault: true, apiKeyEncrypted: null },
      { provider: "anthropic", modelId: "claude", isDefault: false, apiKeyEncrypted: "enc_key" },
    ];
  ...
  it("setModelPreference: updates existing config", async () => {
    modelConfigsMock.findFirst.mockResolvedValueOnce({ id: "mc_1", orgId: "org_test123", provider: "anthropic" });
    const existing = await mockDb.query.modelConfigs.findFirst({ where: "orgId AND provider" });
  ...
  it("getIntegrations: returns connection statuses", async () => {
    ...
    const result = await mockDb.query.mcpConnections.findMany({ where: "orgId" });
    expect(result).toHaveLength(2);
  ...
  it("disconnectIntegration: clears credentials and sets disconnected", async () => {
    mockDb.update(null as any);
    updateChain.set({ credentialsEncrypted: null, status: "disconnected" });
    expect(updateChain.set).toHaveBeenCalledWith(
      expect.objectContaining({ status: "disconnected", credentialsEncrypted: null }),
    );
  });
});
...
describe("queueRouter", () => {
  it("position: returns -1 when job not found", async () => {
    ...
    expect(job).toBeNull();
    const result = { taskId: "task_unknown", position: -1, estimatedWaitSeconds: 0, totalInQueue: 0 };
    expect(result.position).toBe(-1);
  });

  it("position: calculates position for waiting job", async () => {
    const { agentTaskQueue } = await import("@prometheus/queue");
    const mockJob = { id: "task_1", getState: vi.fn().mockResolvedValue("waiting") };
    (agentTaskQueue.getJob as any).mockResolvedValueOnce(mockJob);
    (agentTaskQueue.getWaiting as any).mockResolvedValueOnce([{ id: "task_0" }, { id: "task_1" }]);
    (agentTaskQueue.getWaitingCount as any).mockResolvedValueOnce(3);
  ...
  it("position: returns position 0 for active job", async () => {
    const mockJob = { id: "task_1", getState: vi.fn().mockResolvedValue("active") };
    const { agentTaskQueue } = await import("@prometheus/queue");
    (agentTaskQueue.getJob as any).mockResolvedValueOnce(mockJob);
...
describe("brainRouter", () => {
  it("search: returns matching code embeddings", async () => {
    const results = [{ id: "emb_1", filePath: "src/index.ts", content: "const hello = 'world'", chunkIndex: 0 }];
  ...
  it("getMemories: returns memories filtered by type", async () => {
    const memories = [{ id: "mem_1", memoryType: "semantic", content: "Project uses React" }];
    agentMemoriesMock.findMany.mockResolvedValueOnce(memories);
    const result = await mockDb.query.agentMemories.findMany({ where: "projectId AND type=semantic" });
    expect(result).toHaveLength(1);
    expect(result[0].memoryType).toBe("semantic");
  });

  it("storeMemory: inserts memory and returns it", async () => {
    const memory = { id: "mem_mock123", projectId: "proj_1", memoryType: "convention", content: "Use kebab-case for files" };
  ...
  it("getBlueprint: returns active blueprint with versions", async () => {
    blueprintsMock.findFirst.mockResolvedValueOnce({ id: "bp_1", projectId: "proj_1", isActive: true, versions: [] });
    const blueprint = await mockDb.query.blueprints.findFirst({ where: "projectId AND isActive=true" });
  ...
  it("graph: returns file nodes and empty edges", async () => {
    const files = [
...
describe("fleetRouter", () => {
  it("dispatch: verifies session ownership", async () => {
    sessionsMock.findFirst.mockResolvedValueOnce({ id: "ses_1", projectId: "proj_1", project: { id: "proj_1", orgId: "org_test123" } });
    const session = await mockDb.query.sessions.findFirst({ where: "id" });
    expect(session!.project.orgId).toBe("org_test123");
  });
  ...
  it("dispatch: creates tasks and queues them for each input", async () => {
    const { agentTaskQueue } = await import("@prometheus/queue");
    sessionsMock.findFirst.mockResolvedValueOnce({ id: "ses_1", projectId: "proj_1", project: { id: "proj_1", orgId: "org_test123" } });
    insertChain.returning.mockResolvedValue([{ id: "task_1", title: "Build API", status: "queued" }]);
  ...
  it("status: returns agents and tasks for session", async () => {
    const agents = [
      { id: "agt_1", role: "backend_coder", status: "working", tokensIn: 100, tokensOut: 50, stepsCompleted: 3, startedAt: new Date() },
    ];
    const tasks = [
      { id: "task_1", title: "Build API", status: "completed", agentRole: "backend_coder", creditsConsumed: 5 },
    ];
    agentsMock.findMany.mockResolvedValueOnce(agents);
    tasksMock.findMany.mockResolvedValueOnce(tasks);

    const activeAgents = await mockDb.query.agents.findMany({ where: "sessionId" });
    const sessionTasks = await mockDb.query.tasks.findMany({ where: "sessionId" });
    expect(activeAgents).toHaveLength(1);
    expect(sessionTasks).toHaveLength(1);
  });

  it("stop: terminates specific agent when agentId provided", async () => {
    mockDb.update(null as any);
  ...
  it("stop: terminates all session agents when no agentId", async () => {
    mockDb.update(null as any);
    updateChain.set({ status: "terminated", terminatedAt: expect.any(Date) });
...
describe("userRouter", () => {
  it("profile: returns user with settings", async () => {
    usersMock.findFirst.mockResolvedValueOnce({ id: "usr_1", clerkId: "user_test123", settings: { theme: "dark" } });
    const user = await mockDb.query.users.findFirst({ where: "clerkId", with: { settings: true } });
  ...
  it("updateSettings: updates existing settings", async () => {
    ...
    userSettingsMock.findFirst.mockResolvedValueOnce({ userId: "usr_1", theme: "light" }); // existing settings
    const user = await mockDb.query.users.findFirst({ where: "clerkId" });
    const existing = await mockDb.query.userSettings.findFirst({ where: "userId" });
    expect(user).toBeTruthy();
    expect(existing).toBeTruthy();
  ...
  it("organizations: returns user's org memberships", async () => {
    ...
    orgMembersMock.findMany.mockResolvedValueOnce([
      { role: "admin", organization: { id: "org_1", name: "Acme", slug: "acme", planTier: "pro" } },
    ]);

    const user = await mockDb.query.users.findFirst({ where: "clerkId" });
    const memberships = await mockDb.query.orgMembers.findMany({ where: "userId", with: { organization: true } });
...
describe("integrationsRouter", () => {
  it("list: returns all org integrations", async () => {
    const connections = [
      { id: "int_1", provider: "github", status: "connected", connectedAt: new Date() },
      { id: "int_2", provider: "linear", status: "disconnected", connectedAt: null },
    ];
    mcpConnectionsMock.findMany.mockResolvedValueOnce(connections);
    const result = await mockDb.query.mcpConnections.findMany({ where: "orgId" });
    expect(result).toHaveLength(2);
  ...
  it("connect: updates existing connection", async () => {
    mcpConnectionsMock.findFirst.mockResolvedValueOnce({ id: "int_1", orgId: "org_test123", provider: "github" });
    const existing = await mockDb.query.mcpConnections.findFirst({ where: "orgId AND provider" });
    expect(existing).toBeTruthy();
  ...
  it("disconnect: clears credentials and updates status", async () => {
    mockDb.update(null as any);
    updateChain.set({ credentialsEncrypted: null, status: "disconnected" });
    expect(updateChain.set).toHaveBeenCalledWith(
      expect.objectContaining({ credentialsEncrypted: null, status: "disconnected" }),
    );
  });

  it("getToolConfigs: returns configs for project", async () => {
    const configs = [{ id: "tc_1", toolName: "read_file", enabled: true, configJson: {} }];
    mcpToolConfigsMock.findMany.mockResolvedValueOnce(configs);
    const result = await mockDb.query.mcpToolConfigs.findMany({ where: "projectId" });
    expect(result).toHaveLength(1);
    expect(result[0].toolName).toBe("read_file");
  });

  it("setToolConfig: updates existing tool config", async () => {
    mcpToolConfigsMock.findFirst.mockResolvedValueOnce({ id: "tc_1", projectId: "proj_1", toolName: "read_file" });
    const existing = await mockDb.query.mcpToolConfigs.findFirst({ where: "projectId AND toolName" });
    expect(existing).toBeTruthy();
  });
