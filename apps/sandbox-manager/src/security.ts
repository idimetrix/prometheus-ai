...
export function validateCommand(command: string): { valid: boolean; reason?: string } {
  ...
  for (const pattern of DANGEROUS_PATTERNS) {
    if (pattern.test(command)) {
      ...
      return { valid: false, reason: "Command matches a blocked dangerous pattern" };
    }
  }
  ...
  for (const segment of segments) {
    ...
    if (!COMMAND_ALLOWLIST.has(baseCmd)) {
      logger.warn({ command, baseCommand: baseCmd }, "Blocked non-allowlisted command");
      return { valid: false, reason: `Command '${baseCmd}' is not in the allowlist` };
    }
  }
 ...
 */
export function validateFilePath(sandboxRoot: string, filePath: string): { valid: boolean; reason?: string } {
  ...
  if (!resolved.startsWith(normalizedRoot + path.sep) && resolved !== normalizedRoot) {
    logger.warn({ sandboxRoot, filePath, resolved }, "Blocked path escape attempt");
  ...
  for (const pattern of sensitivePatterns) {
    if (pattern.test(relative) || pattern.test("/" + relative)) {
      // Allow reading .env files but log a warning
      logger.warn({ filePath: relative }, "Accessing sensitive file path");
    }
  }
 ...
 */
export function validateTimeout(timeout: number): { valid: boolean; timeout: number } {
