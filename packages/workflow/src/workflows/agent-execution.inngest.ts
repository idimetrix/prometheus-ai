...
import type { AgentExecutionEvent, WorkflowContext } from "../inngest";
 ...
 * completed step.
 */
export const agentExecutionWorkflow = inngest.createFunction(
  {
    id: "agent-execution",
  ...
  async ({ event, step }: WorkflowContext<AgentExecutionEvent>) => {
    const { taskId, sessionId, taskDescription, mode, agentRole } = event.data;
    ...
    const _architectureResult = await step.run("architecture", () => {
    ...
    for (const planStep of plan) {
      const result = await step.run(`coding-${planStep.id}`, async () => {
        logger.info(
